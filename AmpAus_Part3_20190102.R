...
ampaus$Age <- ifelse(ampaus$Age == "26-30", "26-40",
                     ifelse(ampaus$Age == "31-40", "26-40",ampaus$Age))
###############################################################
...
pd$Age <- ifelse(pd$Age == "17-25", 4,
                 ifelse(pd$Age == "26-40", 3, 
                        ifelse(pd$Age == "41-50", 2, 
                               ifelse(pd$Age == "51-80", 1, pd$Age))))
...
a4150 <- subset(ldd, Age == "41-50")
a5180<- subset(ldd, Age == "51-80")
# apply Function to data sets
lexdiv1725 <- lexdiv(a1725)
lexdiv2640 <- lexdiv(a2640)
lexdiv4150 <- lexdiv(a4150)
lexdiv5180 <- lexdiv(a5180)
# find common items
cmnamps <- Reduce(intersect, list(names(lexdiv1725),names(lexdiv2640),names(lexdiv4150), names(lexdiv5180)))
...
lexdivvls <- data.frame(lexdiv1725[which(names(lexdiv1725) %in% cmnamps)][order(names(lexdiv1725[which(names(lexdiv1725) %in% cmnamps)]))], 
                        ...
                        lexdiv4150[which(names(lexdiv4150) %in% cmnamps)][order(names(lexdiv4150[which(names(lexdiv4150) %in% cmnamps)]))], 
                        lexdiv5180[which(names(lexdiv5180) %in% cmnamps)][order(names(lexdiv5180[which(names(lexdiv5180) %in% cmnamps)]))])
# transpose data
...
p6d$Age <- ifelse(p6d$Age == "17-25", 4,
                  ifelse(p6d$Age == "26-40", 3, 
                         ifelse(p6d$Age == "41-50", 2, 
                                ifelse(p6d$Age == "51-80", 1, p6d$Age))))
...
###############################################################
#                  CVCLA: 41-50
df4150 <- cclad[cclad$Age == "41-50",]
t1 <- tapply(df4150$Amplified, list(df4150$Adjective, df4150$Variant), table)
t2 <- apply(t1, 1, function(x) ifelse(is.na(x) == T, 0, x))
t3 <- t(t2)
t3 <- t3[, 2:ncol(t3)]
t3 <- t3[rowSums(t3) > 0, ]
...
# apply collextbedit Function
collex4150 <- collextbedit(collextab)
# inspect results
subset(collex4150, sig != "n.s.")

###############################################################
#                  CVCLA: 51-80
df5180 <- cclad[cclad$Age == "51-80",]
t1 <- tapply(df5180$Amplified, list(df5180$Adjective, df5180$Variant), table)
t2 <- apply(t1, 1, function(x) ifelse(is.na(x) == T, 0, x))
t3 <- t(t2)
t3 <- t3[, 2:ncol(t3)]
t3 <- t3[rowSums(t3) > 0, ]
...
collextab <- rbind(pretty, really, so, very)
# apply collextbedit Function
collex5180 <- collextbedit(collextab)
# inspect results
subset(collex5180, sig != "n.s.")

###########################################################################
# combine covar collex data frames
collexdfAge <- rbind(collex1725[,c(1:11,15)], collex2640[,c(1:11,15)], 
                     collex4150[,c(1:11,15)], collex5180[,c(1:11,15)])
Age <- c(rep("1725", nrow(collex1725)), rep("2640", nrow(collex2640)), 
         rep("4150", nrow(collex4150)), rep("5180", nrow(collex5180)))
# create data frame
...
# recode Age
p10tb$Age <- ifelse(p10tb$Age == "1725", 4, p10tb$Age)
p10tb$Age <- ifelse(p10tb$Age == "2640", 3, p10tb$Age)
p10tb$Age <- ifelse(p10tb$Age == "4150", 2, p10tb$Age)
p10tb$Age <- ifelse(p10tb$Age == "5180", 1, p10tb$Age)
p10tb$Age <- as.numeric(p10tb$Age)
# create vector with Age groups
Agelbs <- c("17-25", "26-40", "41-50", "51-80")
head(p10tb)
...
# recode Age
p11d$Age <- ifelse(p11d$Age == "17-25", 4, p11d$Age)
p11d$Age <- ifelse(p11d$Age == "26-40", 3, p11d$Age)
p11d$Age <- ifelse(p11d$Age == "41-50", 2, p11d$Age)
p11d$Age <- ifelse(p11d$Age == "51-80", 1, p11d$Age)
...
# reclategorzie Priming
reallyaus$Priming <- ifelse(reallyaus$Priming == "noprime", "NoPrime",
                               ifelse(reallyaus$Priming == "prime", "Prime", reallyaus$Priming))
# reclategorzie Age
reallyaus$Age <- ifelse(reallyaus$Age == "41-50", "41-80",
                            ifelse(reallyaus$Age == "51-80", "41-80", reallyaus$Age))
# reclategorzie SemanticCategory
...
# reclategorzie ConversationType
